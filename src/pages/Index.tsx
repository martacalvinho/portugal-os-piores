...
const Index = () => {
  ...
  const handleVote = async (id: number) => {
    ...
    const {
      ...
      votes: people.find(p => p.id === id)?.votes! + 1
    }).eq('id', id);
    if (updateError) {
  ...
  return <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold tracking-tight text-gray-900 sm:text-5xl md:text-6xl">Top Tugas</h1>
          <p className="mt-3 text-gray-500 sm:mt-4 font-normal text-2xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div className="flex flex-col sm:flex-row gap-4 flex-grow">
            <div className="relative flex-grow max-w-md">
              ...
              <Input type="search" placeholder="Procurar por nome..." className="pl-10" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            <div className="flex gap-2 flex-wrap">
              {categories.map(category => <Badge key={category.value} variant={selectedCategory === category.value ? "default" : "outline"} className="cursor-pointer" onClick={() => setSelectedCategory(category.value)}>
                  {category.label}
                </Badge>)}
            </div>
          </div>
          <div className="flex gap-4 items-center">
            <Button variant={showRecent ? "default" : "outline"} onClick={() => setShowRecent(!showRecent)} className="gap-2">
